namespace BISoft.Ejercicios.Shared.Helpers
{
    public class PagerList<T> : List<T>
    {
        ...
        public static async Task<PagerList<T>> Create(IQueryable<T> source, int pageIndex, int pageSize)
        {
            ...
            //Todo: Implementar el conteo de registros
            var count = 10;// await source.CountAsync();
            var items = new List<T>();// await source.Skip((pageIndex - 1) * pageSize).Take(pageSize).ToListAsync();
